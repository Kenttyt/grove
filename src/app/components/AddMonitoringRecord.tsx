...
const mangroveIcon = L.divIcon({
  ...
  html: `<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40">
    <!-- Outer white border for trunk/roots -->
    <path d="M20 22v7" stroke="#ffffff" stroke-width="6" stroke-linecap="round"/>
    <path d="M20 24c-3 1.5-6 4-8 8" stroke="#ffffff" stroke-width="5" stroke-linecap="round"/>
    <path d="M20 24c3 1.5 6 4 8 8" stroke="#ffffff" stroke-width="5" stroke-linecap="round"/>
    <path d="M20 25v7" stroke="#ffffff" stroke-width="5" stroke-linecap="round"/>
    
    <!-- Canopy (leaves) with white border -->
    <path d="M20 4c4.4 0 8 3.6 8 8 0 .7-.1 1.3-.3 2 2.4.9 4.3 3 4.3 5.5 0 3.3-2.7 6-6 6h-12c-3.3 0-6-2.7-6-6 0-2.5 1.9-4.6 4.3-5.5-.2-.7-.3-1.3-.3-2 0-4.4 3.6-8 8-8z" fill="#2d6a4f" stroke="#ffffff" stroke-width="2.5" stroke-linejoin="round"/>
    
    <!-- Trunk & Roots actual green colors -->
    <path d="M20 22v7" stroke="#40916c" stroke-width="3.5" stroke-linecap="round"/>
    <path d="M20 24c-3 1.5-6 4-8 8" stroke="#52b788" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M20 24c3 1.5 6 4 8 8" stroke="#52b788" stroke-width="2.5" stroke-linecap="round"/>
    <path d="M20 25v7" stroke="#52b788" stroke-width="2.5" stroke-linecap="round"/>
  </svg>`,
  iconSize: [40, 40],
  iconAnchor: [20, 32],
  popupAnchor: [0, -32],
...
const findMatchingBarangay = (name: string): string | null => {
  ...
  for (const b of barangayOptions) {
    ...
    if (target.includes(bClean) || bClean.includes(target)) {
      return b;
    }
  }
...
const fetchBarangayFromCoordinates = async (lat: number, lng: number): Promise<string | null> => {
  try {
    ...
    for (const field of possibleFields) {
      if (typeof field === 'string' && field.trim()) {
        const matched = findMatchingBarangay(field.trim());
        if (matched) return matched;
      }
    }
    
    if (typeof data.display_name === 'string') {
      const matched = findMatchingBarangay(data.display_name);
      if (matched) return matched;
    }
  } catch (error) {
...
export function AddMonitoringRecord() {
  const topRef = useRef<HTMLDivElement | null>(null);
  const formRefs = useRef<Partial<Record<keyof FormState, HTMLElement | null>>>({});
  const [formState, setFormState] = useState<FormState>(initialFormState);
  const [validationErrors, setValidationErrors] = useState<ValidationErrors>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLocatingBarangay, setIsLocatingBarangay] = useState(false);
  ...
  const liveExistingRecords = useMemo(() => {
    return existingRecords
      ...
      .filter((item): item is { record: any; position: [number, number] } => Boolean(item));
  }, [existingRecords]);

  const handleCoordsBlur = async () => {
    const lat = Number(formState.latitude.trim());
    const lng = Number(formState.longitude.trim());
    if (
      !Number.isNaN(lat) &&
      !Number.isNaN(lng) &&
      lat >= -90 && lat <= 90 &&
      ...
      setIsLocatingBarangay(true);
      const barangay = await fetchBarangayFromCoordinates(lat, lng);
      setIsLocatingBarangay(false);
      if (barangay) {
        updateField('barangay', barangay);
      }
    }
  };
  const latitudeValue = Number(formState.latitude);
  const longitudeValue = Number(formState.longitude);
  ...
  const scrollToFirstError = (errors: ValidationErrors) => {
    ...
    const fieldElement = firstField ? formRefs.current[firstField] : null;
  ...
  const getControlClass = (field: keyof FormState) =>
    `w-full rounded-2xl border px-4 py-3 text-sm outline-none transition duration-200 cursor-text ${
      validationErrors[field]
        ? 'border-destructive bg-destructive/5 text-destructive placeholder:text-destructive/50 focus:border-destructive focus:ring-2 focus:ring-destructive/20'
        : 'border-border bg-background placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary/10'
    }`;

  const getErrorMessage = (field: keyof FormState) => validationErrors[field] ? (
    <p className="mt-1 text-xs text-destructive">{validationErrors[field]}</p>
  ) : null;
  ...
  const handleSubmit = async (status: 'draft' | 'published') => {
    ...
    if (!formState.barangay) {
      errors.barangay = 'Barangay is required.';
    }
    if (!formState.latitude.trim()) {
      errors.latitude = 'Latitude is required.';
    } else if (Number.isNaN(Number(formState.latitude.trim()))) {
      errors.latitude = 'Latitude must be a valid number.';
    } else {
      const value = Number(formState.latitude.trim());
    ...
    } else if (Number.isNaN(Number(formState.longitude.trim()))) {
      errors.longitude = 'Longitude must be a valid number.';
    } else {
      const value = Number(formState.longitude.trim());
    ...
    if (!formState.datePlanted) {
      errors.datePlanted = 'Date planted is required.';
    }
    if (!formState.numberSeedlings.trim()) {
      errors.numberSeedlings = 'Number of seedlings is required.';
    } else if (!Number.isInteger(Number(formState.numberSeedlings.trim())) || Number(formState.numberSeedlings.trim()) < 0) {
      errors.numberSeedlings = 'Number of seedlings must be a whole number.';
    }
    ...
    formData.append('site_name', formState.siteName.trim());
    formData.append('barangay', formState.barangay.trim());
    formData.append('latitude', formState.latitude.trim());
    formData.append('longitude', formState.longitude.trim());
    formData.append('species', formState.species.trim());
    formData.append('date_planted', formState.datePlanted);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Unable to save monitoring record.');
      }

      setMessage(result.message || 'Monitoring record saved successfully.');
      setValidationErrors({});
      if (status === 'published') {
        setFormState(initialFormState);
      }
  ...
  return (
    <div className="space-y-8" ref={topRef}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Fill in the details below to add a new mangrove monitoring record. Fields marked with * are required.
          </p>
        </div>
        {message && (
          <div className="rounded-2xl border border-green-200/50 bg-gradient-to-r from-green-50 to-green-50/50 p-4 text-sm text-green-700 flex items-start gap-3 shadow-sm">
            <span className="text-lg flex-shrink-0 mt-0.5">✓</span>
            <span>{message}</span>
          </div>
        )}
        {error && (
          <div className="rounded-2xl border border-destructive/20 bg-gradient-to-r from-destructive/10 to-destructive/5 p-4 text-sm text-destructive flex items-start gap-3 shadow-sm">
            <span className="text-lg flex-shrink-0 mt-0.5">⚠</span>
            <span>{error}</span>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.3fr,0.95fr] gap-6">
        <div className="space-y-6">
          <div className="bg-card rounded-3xl border border-border p-8 shadow-sm hover:shadow-md transition-shadow">
            <div className="mb-6 flex items-start gap-4">
              <div className="size-12 flex-shrink-0 grid place-items-center rounded-2xl bg-gradient-to-br from-primary/20 to-primary/10 text-primary font-bold text-lg">
                1
              </div>
              <div className="flex-1">
                <h2 className="text-lg font-semibold">Location Information</h2>
                <p className="text-sm text-muted-foreground">Site details and GPS coordinates for the new planting location.</p>
              </div>
            </div>
            <div className="space-y-6">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ref={(element) => { formRefs.current.siteName = element; }}
                    value={formState.siteName}
                    onChange={(event) => updateField('siteName', event.target.value)}
                    className={getControlClass('siteName')}
                    placeholder="e.g. Purok 5 Mangrove Area"
                  />
                ...
                <div>
                  ...
                  <select
                    ref={(element) => { formRefs.current.barangay = element; }}
                    value={formState.barangay}
                    disabled={isLocatingBarangay}
                    onChange={(event) => updateField('barangay', event.target.value)}
                    className={getControlClass('barangay') + ' cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed'}
                  >
                ...
                <div className="grid grid-cols-2 gap-4 xl:col-span-2">
                  <div>
                    ...
                    <input
                      type="number"
                      step="any"
                      ref={(element) => { formRefs.current.latitude = element; }}
                      value={formState.latitude}
                      onChange={(event) => updateField('latitude', event.target.value)}
                      onBlur={handleCoordsBlur}
                      className={getControlClass('latitude')}
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      step="any"
                      ref={(element) => { formRefs.current.longitude = element; }}
                      value={formState.longitude}
                      onChange={(event) => updateField('longitude', event.target.value)}
                      onBlur={handleCoordsBlur}
                      className={getControlClass('longitude')}
              ...
              <div className="rounded-2xl border border-border bg-gradient-to-br from-muted/50 to-background p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-base font-semibold">Pin Location on Map</h3>
                    <p className="text-sm text-muted-foreground">Click anywhere on the map to set the coordinates</p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="relative h-[500px] rounded-2xl border border-border overflow-hidden">
                  <MapContainer
                    center={mapCenter}
                  ...
                  >
                    ...
                    />
                    {/* Live Existing Records (circles) */}
                    {liveExistingRecords.map(({ record, position }) => (
                      <CircleMarker
                      ...
                      >
                        <Popup>
                          <div className="p-2 text-xs font-sans text-foreground min-w-[150px]">
                            <h4 className="font-bold text-sm text-primary mb-1">{record.site_name}</h4>
                            <div className="space-y-1 text-muted-foreground">
                              <p><span className="font-semibold text-foreground/80">Barangay:</span> {record.barangay}</p>
                              <p><span className="font-semibold text-foreground/80">Species:</span> {record.species}</p>
                              <p><span className="font-semibold text-foreground/80">Seedlings:</span> {record.number_seedlings}</p>
                              <p>
                                <span className="font-semibold text-foreground/80">Status: </span>
                                <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold ${
                                  record.survival_status === 'Alive' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700'
                                }`}>
                    ...
                    {hasCoordinates && <Marker position={mapCenter} icon={mangroveIcon} />}
                  </MapContainer>
                  {showTileError && (
                    <div className="absolute inset-0 flex items-center justify-center bg-background/80 text-xs text-muted-foreground z-[1000]">
                  ...
                  {hasCoordinates && (
                    <div className="absolute left-3 bottom-3 rounded-xl border border-emerald-200 bg-white/95 px-3 py-2 text-left shadow-sm z-[1000]">
                      <p className="text-[11px] font-semibold text-emerald-700">📍 Location Pinned</p>
                      <p className="text-xs text-emerald-700 font-mono mt-1">{formState.latitude}, {formState.longitude}</p>
                      <button
                        type="button"
          ...
          <div className="bg-card rounded-3xl border border-border p-8 shadow-sm hover:shadow-md transition-shadow">
            <div className="mb-6 flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-lg font-semibold">Planting Details</h2>
                <p className="text-sm text-muted-foreground">Information about the mangrove species and planting method.</p>
              </div>
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold mb-2">Mangrove Species *</label>
                <select
                  ref={(element) => { formRefs.current.species = element; }}
                  value={formState.species}
                  onChange={(event) => updateField('species', event.target.value)}
                  className={getControlClass('species') + ' cursor-pointer'}
                >
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Date Planted *</label>
                <input
                  ref={(element) => { formRefs.current.datePlanted = element; }}
                  type="date"
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Number of Seedlings *</label>
                <input
                  ref={(element) => { formRefs.current.numberSeedlings = element; }}
                  type="number"
                  value={formState.numberSeedlings}
                  onChange={(event) => updateField('numberSeedlings', event.target.value)}
                  className={getControlClass('numberSeedlings')}
                  placeholder="Enter number of seedlings"
                  min="0"
                />
                {getErrorMessage('numberSeedlings')}
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2">Planting Method</label>
                <select
                  value={formState.plantingMethod}
                  onChange={(event) => updateField('plantingMethod', event.target.value)}
                  className={getControlClass('plantingMethod') + ' cursor-pointer'}
                >
                  <option value="" disabled>Select method</option>
                  {plantingMethods.map((option) => (
                    <option key={option} value={option}>{option}</option>
                  ))}
                </select>
              </div>
          ...
          <div className="bg-card rounded-3xl border border-border p-8 shadow-sm hover:shadow-md transition-shadow">
            <div className="mb-6 flex items-start gap-4">
              ...
              <div className="flex-1">
                <h2 className="text-lg font-semibold">Monitoring Information</h2>
                <p className="text-sm text-muted-foreground">Current site measurements and observations.</p>
              </div>
            </div>
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-semibold mb-2">Monitoring Date *</label>
                <input
                  ref={(element) => { formRefs.current.monitoringDate = element; }}
                  type="date"
                  value={formState.monitoringDate}
                  onChange={(event) => updateField('monitoringDate', event.target.value)}
                  className={getControlClass('monitoringDate')}
                />
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-border bg-card p-8 shadow-sm">
            <h3 className="text-lg font-semibold">Submit Record</h3>
            <p className="text-sm text-muted-foreground mb-6">Save the monitoring record to the backend database.</p>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <button
                type="button"
                disabled={isSubmitting}
                onClick={() => handleSubmit('draft')}
                className="inline-flex items-center justify-center rounded-2xl border border-border bg-background px-6 py-3 text-sm font-semibold transition duration-200 cursor-pointer hover:border-primary hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed"
              >
                💾 Save as Draft
              </button>
              <button
                ...
                className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-primary to-primary/90 px-6 py-3 text-sm font-semibold text-primary-foreground transition duration-200 cursor-pointer hover:from-primary/90 hover:to-primary shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ✓ Save Record
              </button>
              <button
                ...
                disabled={isSubmitting}
                onClick={resetForm}
                className="inline-flex items-center justify-center rounded-2xl border border-border bg-background px-6 py-3 text-sm font-semibold text-muted-foreground transition duration-200 cursor-pointer hover:border-primary hover:text-foreground disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ✕ Cancel
              </button>
            </div>
